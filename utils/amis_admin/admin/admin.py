...
class BaseModelAdmin(SQLModelCrud):
    ...
    async def get_list_table(self, request: Request) -> TableCRUD:
        ...
        table = TableCRUD(
            ...
            footerToolbar=[{"type": "switch-per-page", "align": "left"},
                           ...
                           {"type": "tpl",
                            "tpl": "SHOWING ${items|count} OF ${total} RESULT${IF(total>1,'S','')}",
    ...
    async def get_form_item_on_foreign_key(
        ...
        picker = Picker(
            ...
            #valueField="id"
            #zhaoyang fixed
            labelField=admin.pk_label,
            valueField=admin.pk_name,
    ...
    async def get_form_item(
        ...
        return await self.get_form_item_on_foreign_key(request, modelfield,
                                                       is_filter=is_filter) or self.amis_parser.as_form_item(
    ...
    async def get_read_form(self, request: Request) -> Form:
        return Form(
            ...
            name=CrudEnum.read,
            body=await self._conv_modelfields_to_formitems(request, self.schema_read.__fields__.values(),
                                                           CrudEnum.read),
            submitText=None,
            static=True,
        )
    ...
    async def get_read_action(self, request: Request) -> Optional[Action]:
        ...
        return ActionType.Drawer(
            icon="fa fa-eye",
            className="datetable-action-view-btn-cls",
            tooltip=_("View"),
    ...
    async def get_update_action(self, request: Request, bulk: bool = False) -> Optional[Action]:
        ...
        if not bulk:
            return ActionType.Drawer(
                icon="fa fa-pencil",
                className="datetable-action-edit-btn-cls",
                tooltip=_("Update"),
                drawer=Drawer(
...
class ModelAdmin(BaseModelAdmin, PageAdmin):
    ...
    async def has_create_permission(self, request: Request, data: SchemaCreateT,
                                    **kwargs) -> bool:  # type self.schema_create
        return await self.has_page_permission(request)
...
class ModelAction(BaseFormAdmin, BaseModelAction):
    ...
    async def handle(self, request: Request, item_id: List[str], data: Optional[SchemaUpdateT], **kwargs) -> BaseApiOut[
        Any]:
        return BaseApiOut(data=data)
...
class AdminApp(PageAdmin, AdminGroup):
    ...
    async def _get_page_as_tabs(self, request: Request) -> Page:
        ...
        page.body = PageSchema(children=children).as_tabs_item(
            tabs_extra=dict(tabsMode=self.tabs_mode, mountOnEnter=True)).tab
        return page
